...
- `concurrency-architecture` — removing misplaced `@MainActor` from Repository/APIClient/Logger (replace with Sendable types crossing the boundary), replacing `Task.detached` with structured `Task { }` inside the layered chain, hoisting screen-bound long-running work (uploads, sync) into an app-scoped Service registered in DI, replacing `actor`-as-Singleton (`static let shared`) with DI-wired actor, removing `await MainActor.run { ... }` boilerplate by marking the calling class `@MainActor` once, splitting `Task.sleep`-based timeouts into proper `withTimeout` at UseCase boundary or `URLSessionConfiguration.timeoutIntervalForRequest`. Defer language-level Swift 6 migration to `swift-concurrency:swift-concurrency` (AvdLee skill)
